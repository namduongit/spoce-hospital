...
const PrescriptionInvoicePayment = () => {
    ...
    const handleGetPrescriptionInvoice = async () => {
        const restResponse = await execute(getPrescriptionInvoiceById(Number(id)));
        showError(restResponse);
        if (restResponse?.result) setServiceInvoice(restResponse.data as PrescriptionInvoiceResponse);
    }

    useEffect(() => {
        id && handleGetPrescriptionInvoice();
    }, [id])
                    ...
                    <div className="bg-white rounded-lg shadow-md p-6">
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="flex justify-between">
                                ...
                                <span className="text-gray-900">{serviceInvoice.createAt && formatDateToHourAndDay(new Date(serviceInvoice.createAt))}</span>
                            </div>
                            <div className="flex justify-between mt-2">
                                <span className="text-gray-600">Cập nhật:</span>
                                <span className="text-gray-900">{serviceInvoice.updateAt && formatDateToHourAndDay(new Date(serviceInvoice.updateAt))}</span>
                            </div>
                            ...
                            <div className="border-t pt-4 mt-4">
                                <h3 className="text-lg font-semibold text-gray-800 mb-3">Thông tin bác sĩ</h3>
                                <div className="space-y-3">
                                    <div className="flex justify-between">
                                        <span className="text-gray-600">Mã bác sĩ:</span>
                                        <span className="text-gray-900 font-medium">{serviceInvoice.doctorId}</span>
                                    </div>
                                    <div className="flex justify-between">
                                        <span className="text-gray-600">Email bác sĩ:</span>
                                        <span className="text-gray-900 font-medium">{serviceInvoice.doctorEmail || "Không có email"}</span>
                                    </div>
                                </div>
                            </div>

                            {serviceInvoice.symptoms && (
                                <div className="border-t pt-4 mt-4">
                                    <h3 className="text-lg font-semibold text-gray-800 mb-3">Triệu chứng</h3>
                                    <p className="text-gray-900">{serviceInvoice.symptoms}</p>
                                </div>
                            )}
                            ...
                            {serviceInvoice.note && (
                                <div className="border-t pt-4 mt-4">
                                    ...
                                    <p className="text-gray-900">{serviceInvoice.note}</p>
                                </div>
                            )}

                            <div className="border-t pt-4 mt-4">
                                <h3 className="text-lg font-semibold text-gray-800 mb-3">Chi tiết đơn thuốc</h3>
                                <div className="space-y-3">
                                    {serviceInvoice.medicines?.map((medicine, index) => (
                                            ...
                                            <div className="flex justify-between items-start mb-2">
                                                <div className="flex-1">
                                                    ...
                                                    {medicine.usageInstructions && (
                                                        <div className="text-sm text-gray-600">
                                                            Cách dùng: {medicine.usageInstructions}
                                                        </div>
                                                    )}
                                                </div>
                                                <div className="text-right ml-4">
                                                    <div className="text-sm text-gray-600">
                                                        {formatPriceVND(medicine.unitPrice)} x {medicine.quantity}
                                                    </div>
                                                    <div className="text-blue-600 font-semibold mt-1">
                                                        {formatPriceVND(medicine.totalPrice)}
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
...
export default PrescriptionInvoicePayment;