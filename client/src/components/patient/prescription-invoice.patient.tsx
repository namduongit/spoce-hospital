...
import PrescriptionInvoiceDetail from "./others/prescription-invoice-detail";

type PrescriptionInvoicePatientProps = {
    prescriptionInvoices: PrescriptionInvoiceResponse[]
}

const PrescriptionInvoicePatient = (props: PrescriptionInvoicePatientProps) => {
    const { prescriptionInvoices } = props;

    const [row, setRow] = useState<number>(5);
    const [page, setPage] = useState<number>(1);
    const [showDetail, setShowDetail] = useState<boolean>(false);
    const [invoiceSelect, setInvoiceSelect] = useState<PrescriptionInvoiceResponse | null>(null);
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                {(prescriptionInvoices && prescriptionInvoices.length > 0) ? (
                    <div className="space-y-4">
                        {prescriptionInvoices.slice((page - 1) * row, page * row).map((invoice, index) => (
                        ...
                        <TablePagination array={prescriptionInvoices} row={row} page={page} setRow={setRow} setPage={setPage} />
                    </div>
                ) : (
            ...
            {(showDetail && invoiceSelect) && (
                <PrescriptionInvoiceDetail 
                    prescriptionInvoiceSelect={invoiceSelect} 
...
export default PrescriptionInvoicePatient;